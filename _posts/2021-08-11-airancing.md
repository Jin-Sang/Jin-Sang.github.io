...
인간이 목적지의 경로를 파악하고(3장), 순간순간 목표를 확인하고(4장), 실제 차량을 모는 것(5장)
과 같은 순서로 알아보았습니다. 
...
훈련을 시킨다면 어떤 식으로 해야할지 고민이 되었습니다.<br/> 
  * 3장 웨이포인트들을 연결하는 최적의 곡선 경로를 찾는 것
    (경로가 주어지면 안되므로 해당 스크립트가 없어야 한다?) 
  * 5장 AI가 경로를 최대한 잘 따라서 주행하게 하는 것
    (주행 방식이 주어지면 안되므로 해당 스크립트가 없어야 한다?)
